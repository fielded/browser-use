...
async def run_agent(browser: Browser, browser_context):
    try:
        # Load tasks
        order_data = load_task_list_from_yaml("tasks/PSM/route_forwarding.yaml")

        if not isinstance(order_data, dict) or "tests" not in order_data:
            raise ValueError("route_forwarding.yaml must contain a 'tests' key with a list of steps.")
            ...
            if isinstance(results, list):
                for res in results:
                    if hasattr(res, "extracted_content") and isinstance(res.extracted_content, str):
                        ...
                        soft_fail_terms = [
                            "dns", "operation failed", "not resolved", "error",
                            "cannot submit"
                            ]
                        if any(term in text for term in soft_fail_terms):
                            log_error_to_markdown(
                                error=res.extracted_content,
                                context="⚠️ Issue with PSM route forwarding",  # check on steps completed.
